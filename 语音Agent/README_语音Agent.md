...
项目统一使用 `arm_rebot` 环境里的 Python：

```bash
mamba run -n arm_rebot python -V
mamba run -n arm_rebot python -m pip install requests pyyaml numpy sounddevice
```
...
```bash
mamba run -n arm_rebot python -m pip install openai
```
...
```bash
cd 机械臂/Web控制台
mamba run -n arm_rebot python 启动Web服务.py
```
...
```bash
mamba run -n arm_rebot python 语音Agent主程序_main.py shell
```

文本 ask：

```bash
mamba run -n arm_rebot python 语音Agent主程序_main.py ask 帮我看看当前机械臂状态
```

语音单回合：

```bash
mamba run -n arm_rebot python 语音Agent主程序_main.py voice
```

listen 长驻模式：

```bash
mamba run -n arm_rebot python 语音Agent主程序_main.py listen --warmup
```

只播报一句话：

```bash
mamba run -n arm_rebot python 语音Agent主程序_main.py say 欢迎来到展会现场
```

warmup：

```bash
mamba run -n arm_rebot python 语音Agent主程序_main.py warmup
```

reset session：

```bash
mamba run -n arm_rebot python 语音Agent主程序_main.py reset-session
```

禁用 TTS：

```bash
mamba run -n arm_rebot python 语音Agent主程序_main.py --no-tts ask 帮我看看当前机械臂状态
```
...
```bash
mamba run -n arm_rebot python 测试脚本_test/01_文本ask测试.py
mamba run -n arm_rebot python 测试脚本_test/02_录音测试.py
mamba run -n arm_rebot python 测试脚本_test/03_STT测试.py
mamba run -n arm_rebot python 测试脚本_test/04_TTS测试.py
mamba run -n arm_rebot python 测试脚本_test/05_工具桥接dry_run测试.py
ARM_MOCK_STT_TEXT=请查询机械臂状态 mamba run -n arm_rebot python 测试脚本_test/06_语音回合测试.py
```
