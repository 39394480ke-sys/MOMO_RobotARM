...
> 注意：这是历史发布草稿，内容可能已经过期。当前项目入口、硬件事实和运行命令请优先查看仓库根目录 `README.md`。本文件保留用于追溯阶段十二发布资料，不应作为当前真机控制依据。

当前默认型号为 V2，主链为 J10-J15，使用 `URDF运动学仿真/urdf/v2/soarmoce_urdf.urdf`
和 `Link_7`；V1 使用 `urdf/v1` 和 `Link_6`。真机验收只看 `docs/V2真机验收.md`。

## 项目目标

本项目是一个学习型机械臂控制项目，目标是从仿真控制、真实舵机、URDF、动作录制、GUI、Web、视觉、语音 Agent 到系统集成，逐步搭建一套可理解、可运行、可扩展的机械臂系统。

本项目包含自主编写的控制、标定、安全检查和集成代码；部分硬件参数、URDF/STL 资源和结构约定参考第三方 SOARM MOCE 资料。重新发布或商用前请核验第三方资料的许可证和署名要求。真实机械臂控制有风险，默认使用 dry-run。
...
阶段三到阶段十一主要搭建软件能力。阶段十二把这些能力整理为装配和维护资料包，说明硬件是什么、怎么装、怎么接线、怎么标定、怎么启动、怎么测试、怎么排查。
...
更多见 `硬件与装配资料/故障排查手册.md`。

## 后续计划

- 补齐第三方 STEP / 3MF / 打印参数来源。
- 补拍装配、接线、标定姿态照片。
- 补充真实硬件电源和螺丝规格。
- 增加发布版本号和变更记录。
- 完善真实机械臂长期稳定性测试。
