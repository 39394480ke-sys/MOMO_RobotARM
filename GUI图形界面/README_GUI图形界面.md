...
本项目当前使用 `momo_rebot` 环境里的 Python：
...
for name in ["PyQt5", "pybullet", "yaml", "numpy", "lerobot"]:
    try:
        __import__(name)
        print(name, "OK")
...
当前 GUI 代码已经使用导轨版关节顺序：`J10/j10` 到 `J15/j15`。
真实模式连接前，阶段四标定文件必须包含 J10-J15 的完整标定；如果 J10 导轨刚装好但还没有记录 `home_present_raw`，真实模式会提示标定不完整并禁止连接。
...
- 输入 J10-J15 六个关节值计算 FK，其中 J10 是导轨位移，单位 mm。
...
导轨版还需要确认 `j10` 标定项存在。J10 的 `home_present_raw` 应该在导轨处于安全中心或你希望作为 Home 的位置时记录，不能直接沿用空载测试位置。
