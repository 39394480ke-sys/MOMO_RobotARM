...
## 8. 为什么 J10/J11/J12/J13/J15 要特殊处理
...
J11 已改为 1:5 减速多圈底座旋转，不再走单圈手动限位标定。旧标定文件里如果仍有 J11 单圈记录，真实移动前请重新运行 `标定程序_calibrate.py` 生成新的 J11 `home_present_raw`。
...
`单圈目标 raw 超出标定范围`：J14 或夹爪目标 raw 超过 `range_min/range_max`。

`多圈目标 raw 超出 signed absolute raw 安全范围`：J10/J11/J12/J13/J15 目标 raw 超过 `-30719 到 30719`。
